...
class Sisme_Submission_Database {
    ...
    public static function update_submission_status($submission_id, $new_status, $user_id = null) {
        global $wpdb;
        ...
        // Vérifier permissions si user_id fourni
        if ($user_id && $submission->user_id != $user_id) {
            return new WP_Error('permission_denied', 'Permission refusée');
        }
        ...
        // Ajouter submitted_at si passage en pending
        if ($new_status === self::STATUS_PENDING && $submission->status !== self::STATUS_PENDING) {
            $update_data['submitted_at'] = current_time('mysql');
            $update_format[] = '%s';
        }
        
        // Ajouter published_at si passage en published
        if ($new_status === self::STATUS_PUBLISHED && $submission->status !== self::STATUS_PUBLISHED) {
        ...
        if ($result === false) {
            return new WP_Error('db_error', 'Erreur lors de la mise à jour du statut');
        }
        
        // Nettoyer le cache
        wp_cache_delete("submission_$submission_id", 'sisme_submissions');
        wp_cache_delete("user_submissions_$submission->user_id", 'sisme_submissions');
        
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("[Sisme Submissions] Statut mis à jour : ID $submission_id, nouveau statut: $new_status");
        }
    ...
    /**
     * Méthode utilitaire pour vérifier si une soumission est éditable
     */
    public static function is_editable($submission_id, $user_id = null) {
        $submission = self::get_submission($submission_id);
        ...
        // Seuls les brouillons et révisions sont éditables
        return in_array($submission->status, [self::STATUS_DRAFT, self::STATUS_REVISION]);
    }

    /**
     * Méthode utilitaire pour vérifier les limites utilisateur
     */
    public static function can_user_create_submission($user_id) {
        global $wpdb;
        
        $table_name = $wpdb->prefix . self::TABLE_NAME;
        
        // Vérifier limite de brouillons
        $draft_count = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table_name WHERE user_id = %d AND status IN ('draft', 'revision')",
            $user_id
        ));
        
        if ($draft_count >= self::MAX_DRAFTS_PER_USER) {
            return new WP_Error('limit_exceeded', 'Limite de brouillons atteinte (' . self::MAX_DRAFTS_PER_USER . ' maximum)');
        }
        
        // Vérifier limite de soumissions par jour
        $today_submissions = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table_name WHERE user_id = %d AND DATE(created_at) = CURDATE()",
            $user_id
        ));
        ...
        if ($today_submissions >= self::MAX_SUBMISSIONS_PER_DAY) {
            return new WP_Error('limit_exceeded', 'Limite de soumissions par jour atteinte (' . self::MAX_SUBMISSIONS_PER_DAY . ' maximum)');
        }
